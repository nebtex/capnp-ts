...
tap.test('Message.getFramedSegments()', (t) => {
  ...
  ]).buffer), undefined, 'should throw when message is truncated');
...
tap.test('Message.allocateSegment()', (t) => {
  ...
  t.equal(
    m1.getSegment(0).buffer.byteLength, length * 2 + C.MIN_SINGLE_SEGMENT_GROWTH, 'should replace existing segments');
